-- Fix net worth sign - add negative to flip the sign
-- The current calculation is backwards, so we just need to negate it
...
BEGIN
  ...
  SELECT
    -- Fix: Just negate the current result to flip the sign
