-- Exclude 'Done' status tasks from unscheduled tasks
-- Match the status filtering from the Tasks Hub

DROP FUNCTION IF EXISTS get_unscheduled_tasks(UUID, TEXT);
...
BEGIN
  ...
  WHERE t.user_id = p_user_id
    AND t.status NOT IN ('completed', 'cancelled', 'Done')  -- Exclude completed tasks (both old and new status)
