-- Fix ORDER BY clause in unusual spending detection
-- The ORDER BY was in the wrong place causing GROUP BY errors
  ...
  unusual AS (
    ...
    anomalies AS (
      SELECT
        ...
        cms.current_total,
        ca.avg_monthly,
        ROUND(((cms.current_total - ca.avg_monthly) / NULLIF(ca.avg_monthly, 0)) * 100, 1) as increase_pct,
        (cms.current_total - ca.avg_monthly) as diff_amount
    ...
    FROM (
      SELECT category, current_total, avg_monthly, increase_pct
      FROM anomalies
