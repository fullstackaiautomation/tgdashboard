-- Fix net worth calculation to be Total Assets - Total Debt
-- Net Worth = (Cash + Investments) - (Credit Cards + Loans + Taxes)
...
BEGIN
  ...
  WITH net_worth_calc AS (
    SELECT
      -- Total Assets: Cash + Investments (all asset accounts)
      COALESCE(SUM(bs.balance) FILTER (
        WHERE a.account_type IN ('cash', 'investment')
      ), 0) as total_assets,
      -- Total Debt: Credit Cards + Loans + Taxes (all liability accounts)
      COALESCE(SUM(bs.balance) FILTER (
        WHERE a.account_type IN ('credit_card', 'loan', 'tax')
      ), 0) as total_debt
  ...
  SELECT
    ...
    nw.total_assets - nw.total_debt as net_worth,
    mf.revenue,
    mf.expenses,
    bd.budget_amount,
