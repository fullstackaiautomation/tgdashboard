...
- [ ] Task 1: Create enhanced Daily area aggregation query (AC: 1, 4, 5)
  ...
  - [ ] Join with deep_work_sessions for today's logged hours
...
- [ ] Task 4: Implement Deep Work session display (AC: 1, 7)
  - [ ] Query most recent deep_work_sessions WHERE start_time >= TODAY
  - [ ] Calculate session duration: end_time - start_time in hours
  - [ ] Display format: "Last session: 2.5h on Huge Capital (30 min ago)"
  - [ ] Show business/area color-coded chip for visual association
...
- [ ] Task 10: Integrate real-time updates for Daily card (AC: 7)
  ...
  - [ ] Subscribe to deep_work_sessions changes WHERE start_time >= TODAY
...
- Story 4.1: deep_work_sessions table with business_id, duration_minutes
- Daily time allocation: SUM(duration_minutes) WHERE start_time >= TODAY
- Deep Work target tracking (default 6h per day)
- useDeepWorkSessions hook for querying sessions
...
BEGIN
  ...
  deep_work_today AS (
    SELECT
      COALESCE(SUM(dws.duration_minutes) / 60.0, 0) as hours_today,
      b.name as business_name,
      dws.duration_minutes / 60.0 as session_duration,
      dws.end_time as session_time,
      ROW_NUMBER() OVER (ORDER BY dws.end_time DESC) as rn
  ...
  SELECT
    ...
    (SELECT business_name FROM deep_work_today WHERE rn = 1),
