...
## Tasks / Subtasks
- [ ] Task 1: Create useDailyProgress hook for daily metrics (AC: 1, 2, 3)
...
- [ ] Task 2: Build DailyProgressHeader component (AC: 2)
...
- [ ] Task 3: Add "Due Today" card with progress display (AC: 3)
  - [ ] Update src/components/daily/DueTodayCard.tsx (or create if doesn't exist)
  - [ ] Call useDailyProgress() to get completion data
  - [ ] Display format: "X of Y tasks complete - Z%"
  - [ ] Example: "2 of 8 tasks complete - 25%"
  - [ ] Show small ProgressBar (size='sm') below text
  - [ ] List due tasks with checkboxes (link to task detail/edit)
  - [ ] Highlight overdue tasks that are also due today in red
- [ ] Task 4: Implement Deep Work goal progress tracking (AC: 4)
  - [ ] Create src/hooks/useDeepWorkProgress.ts hook
  - [ ] Query deep_work_sessions for current date
  - [ ] Sum duration_minutes for total hours worked today
  - [ ] Fetch daily goal hours from user settings (default: 6 hours)
  - [ ] Calculate progress: (hours_worked / hours_goal) × 100
  - [ ] Return { hoursWorked, hoursGoal, deepWorkProgress }
  - [ ] Display in Deep Work section: "4.5 / 6.0 hours - 75%"
  - [ ] Use ProgressBar to visualize deep work progress
- [ ] Task 5: Build time allocation breakdown by business (AC: 5)
  - [ ] Create src/components/daily/TimeAllocation.tsx component
  - [ ] Query deep_work_sessions grouped by business_id for current date
  - [ ] Calculate % of day per business: (business_minutes / total_minutes) × 100
  - [ ] Display as horizontal stacked bar chart with business colors
  - [ ] Show business name and percentage: "Huge Capital: 45%", "Full Stack AI: 30%", "S4: 25%"
  - [ ] Use business-specific colors from theme.css
  - [ ] Add tooltip on hover showing exact hours per business
  - [ ] Handle case: no deep work logged shows "No time logged yet"
- [ ] Task 6: Enable real-time daily progress updates (AC: 6)
  - [ ] Verify useRealtimeSync hook triggers daily progress recalculation
  - [ ] Subscribe to task updates where due_date = current date
  - [ ] Subscribe to deep_work_sessions inserts/updates for current date
  - [ ] Test: complete task in one tab → verify daily progress updates in another tab
  - [ ] Verify optimistic updates work for instant feedback
  - [ ] Measure sync latency: must be <500ms
- [ ] Task 7: Create end-of-day summary component (AC: 7)
  - [ ] Create src/components/daily/EndOfDaySummary.tsx component
  - [ ] Display after 6pm local time (configurable in user settings)
  ...
  - [ ] Add reflection prompt: "What went well today?" (optional text input)
  - [ ] Include "Plan Tomorrow" button linking to tomorrow's Daily page
  - [ ] Save summary to database for historical tracking
- [ ] Task 8: Implement streak tracking for daily completion (AC: 8)
  - [ ] Create src/hooks/useDailyStreak.ts hook
  - [ ] Query tasks for last 30 days, calculate completion % per day
  - [ ] Identify consecutive days with >80% completion
  - [ ] Calculate current streak: count consecutive days from today backwards
  - [ ] Calculate longest streak: max consecutive days ever achieved
  - [ ] Return { currentStreak, longestStreak, streakActive: boolean }
  - [ ] Display in DailyProgressHeader: "🔥 5-day streak!"
  - [ ] Show celebratory message for new streak records
  - [ ] Add streak history visualization (optional): calendar heatmap showing daily completion
