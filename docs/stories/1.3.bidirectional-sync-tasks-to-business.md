...
## Tasks / Subtasks
- [ ] Task 1: Enhance Tasks Hub inline editing to support all editable fields (AC: 1, 2)
  - [ ] Extend TaskCard.tsx to support editing title, description, due_date, status
  - [ ] Add inline dropdown for changing Project/Phase assignment
  - [ ] Update useTasks.ts mutation to handle all field updates
  - [ ] Ensure updates include updated_at timestamp for conflict resolution
  - [ ] Validate edits before submission (e.g., non-empty title, valid due date)
- [ ] Task 2: Implement automatic sync from Tasks Hub to Business pages (AC: 1, 2, 3)
  - [ ] When task is updated in Tasks Hub, UPDATE operation automatically syncs to Business pages via shared tasks table
  - [ ] Business pages use same useRealtimeSync hook to receive updates
  - [ ] Verify Business page task lists update within 500ms
  - [ ] When task is deleted in Tasks Hub, DELETE operation removes from Business page
  - [ ] Add loading states during sync operations
- [ ] Task 3: Implement conflict resolution with last-write-wins strategy (AC: 4)
  - [ ] Add updated_at timestamp comparison in useMutation onMutate
  - [ ] If local version is stale (updated_at < server updated_at), show conflict warning
  - [ ] Display modal: "This task was updated by another session. Overwrite changes?"
  - [ ] Options: "Keep my changes" (force update) or "Discard my changes" (revert)
  - [ ] Log conflicts to sync logger for debugging
- [ ] Task 4: Add referential integrity validation (AC: 5)
  - [ ] Validate project_id and phase_id exist before allowing task update
  - [ ] Prevent orphaning tasks: if project/phase is deleted, show warning before task deletion
  - [ ] Add database foreign key constraints with ON DELETE CASCADE or ON DELETE SET NULL
  - [ ] Handle FK constraint violations with user-friendly error messages
  - [ ] Show "Cannot move task: Project no longer exists" if target project was deleted
- [ ] Task 5: Implement Project/Phase reassignment in Tasks Hub (AC: 6)
  - [ ] Add "Move to Project/Phase" dropdown in TaskCard.tsx
  - [ ] Fetch available projects and phases using useProjects hook
  - [ ] Update task with new project_id and phase_id on selection
  - [ ] Verify task appears in new location in Business page hierarchy
  - [ ] Update breadcrumb display in TaskCard after reassignment
- [ ] Task 6: Build undo functionality for task changes (AC: 7)
  - [ ] Create src/hooks/useUndo.ts hook for managing undo state
  - [ ] Store previous task state in local state for 30 seconds after edit
  - [ ] Show "Undo" toast notification after task edit with 30-second countdown
  - [ ] Clicking "Undo" reverts task to previous state using stored snapshot
  - [ ] Clear undo state after 30 seconds or when user navigates away
  - [ ] Undo applies to: status changes, title edits, due date changes, Project/Phase moves
- [ ] Task 7: Add visual sync status indicators (AC: 1)
  - [ ] Show subtle spinner on TaskCard during sync operation
  - [ ] Display checkmark icon when sync completes successfully
  - [ ] Show error icon if sync fails with tooltip explaining error
  - [ ] Add global sync status in Tasks Hub header (Connected/Syncing/Error)
  - [ ] Use Tailwind pulse animation for syncing state
- [ ] Task 8: Test bidirectional sync in multi-tab scenario (AC: all)
