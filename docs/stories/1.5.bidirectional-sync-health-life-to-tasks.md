...
## Tasks / Subtasks
- [ ] Task 1: Create life_areas table and update tasks schema (AC: 1, 2, 3, 5)
  - [ ] Verify life_areas table exists from Story 1.1 (should have id, name, color, category)
  - [ ] Add life_area_id column to tasks table if not already present (nullable FK)
  - [ ] Seed life_areas table with predefined areas: Health, Content, Personal, Shopping, Travel, Journal
  - [ ] Set color values: Health=teal (#14b8a6), Content=purple (#a855f7), Personal=pink (#ec4899)
  - [ ] Update TypeScript LifeArea type in src/types/life-area.ts
- [ ] Task 2: Build Health Dashboard with task creation (AC: 1)
...
- [ ] Task 3: Build Content Dashboard with task creation (AC: 2)
...
- [ ] Task 4: Build Life Areas Dashboard with task creation (AC: 3)
...
- [ ] Task 5: Implement automatic sync from Health/Life pages to Tasks Hub (AC: 1, 2, 3, 4)
  - [ ] When task is created in Health page, INSERT into tasks table with life_area_id = Health
  - [ ] When task is created in Content page, INSERT with life_area_id = Content
  - [ ] When task is created in Life page, INSERT with appropriate life_area_id (Journal, Shopping, etc.)
  - [ ] Verify tasks appear in Tasks Hub within 500ms using real-time subscriptions
  - [ ] Tasks Hub displays life_area badge with color coding
- [ ] Task 6: Implement automatic sync from Tasks Hub to Health/Life pages (AC: 4)
  - [ ] When task is updated in Tasks Hub, UPDATE syncs to Health/Life pages via shared tasks table
  - [ ] Health/Life pages use useRealtimeSync hook to subscribe to tasks table changes
  - [ ] Filter real-time subscriptions by life_area_id to only receive relevant tasks
  - [ ] Verify sync latency <500ms in both directions
  - [ ] Test completion, deletion, and editing in both directions
- [ ] Task 7: Add visual differentiation for Health/Life tasks in Tasks Hub (AC: 5, 6)
  - [ ] Display life_area badge with color: teal for Health, pink for Personal, purple for Content
  - [ ] Do NOT display Project > Phase hierarchy for Health/Life tasks (field is null)
  - [ ] Show life_area name instead: "Health: Workout", "Personal: Shopping", "Content: Library"
  - [ ] Use CSS custom properties from theme.css for area colors
  - [ ] Add icon for each life area type (dumbbell for Health, heart for Personal, book for Content)
- [ ] Task 8: Integrate time tracking for Health/Life tasks (AC: 7)
...
- [ ] Task 9: Test Health/Life and Tasks Hub synchronization (AC: all)
