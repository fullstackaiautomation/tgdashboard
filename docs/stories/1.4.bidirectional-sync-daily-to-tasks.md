...
## Tasks / Subtasks
- [ ] Task 1: Update tasks database schema for Daily page integration (AC: 1, 2)
  - [ ] Add scheduled_date column to tasks table (nullable, allows scheduling for specific day)
  - [ ] Add scheduled_time column to tasks table (nullable, for Daily Schedule integration)
  - [ ] Add recurrence_pattern column (enum: 'none', 'daily', 'weekly', 'monthly', nullable)
  - [ ] Add recurrence_parent_id column (nullable FK to tasks.id for recurring task instances)
  - [ ] Update TypeScript Task type to include new fields
  - [ ] Run database migration to add columns
- [ ] Task 2: Enhance Daily To-Do List to sync tasks with Tasks Hub (AC: 1, 3)
  - [ ] Update src/components/daily/TodoList.tsx to use tasks table instead of local state
  - [ ] When task is added to Daily To-Do List, INSERT into tasks table with scheduled_date = current_date
  - [ ] Auto-tag tasks with scheduled_date so they appear in Tasks Hub filtered by date
  - [ ] When task is completed in Daily To-Do List, UPDATE tasks.status = 'completed'
  - [ ] Verify completion syncs to Tasks Hub and Business pages (if task has business_id)
- [ ] Task 3: Enhance Daily Schedule to sync tasks with Tasks Hub (AC: 2)
  - [ ] Create src/components/daily/Schedule.tsx component for daily schedule view
  - [ ] Allow adding tasks to Daily Schedule with scheduled_time (e.g., "9:00 AM - Review PRD")
  - [ ] INSERT tasks with both scheduled_date and scheduled_time when added to Schedule
  - [ ] Display tasks in time-ordered list in Schedule view
  - [ ] Sync scheduled tasks to Tasks Hub with time badge display
- [ ] Task 4: Add date assignment feature in Tasks Hub (AC: 4)
  - [ ] Add "Schedule for..." button/dropdown in TaskCard.tsx
  - [ ] Implement date picker using HTML5 input type="date" or date-fns + custom picker
  - [ ] Update task with scheduled_date when date is selected
  - [ ] Optionally add scheduled_time picker for precise scheduling
  - [ ] Verify task appears in Daily page after assignment
- [ ] Task 5: Implement automatic Daily page filtering (AC: 5, 7)
  - [ ] Update Daily To-Do List query to filter tasks WHERE scheduled_date = current_date OR due_date = current_date
  - [ ] Create "Due Today" section: tasks WHERE due_date = CURRENT_DATE
  - [ ] Create "Due Tomorrow" section: tasks WHERE due_date = CURRENT_DATE + 1
  - [ ] Display only date-relevant tasks in Daily page (hide tasks without scheduled_date/due_date)
  - [ ] Tasks Hub shows all tasks regardless of scheduled_date (global view)
- [ ] Task 6: Implement recurring task generation (AC: 6)
  - [ ] Create src/utils/recurringTasks.ts utility for generating task instances
  - [ ] When recurring task is created, set recurrence_pattern ('daily', 'weekly', 'monthly')
  - [ ] Generate task instances for next 30 days based on recurrence_pattern
  - [ ] Each instance has recurrence_parent_id pointing to parent task
  - [ ] Display recurring task badge in TaskCard with recurrence icon
  - [ ] Allow completing individual instances without affecting parent or future instances
- [ ] Task 7: Add visual differentiation for Daily tasks in Tasks Hub (AC: 1, 2)
  - [ ] Show calendar icon badge for tasks with scheduled_date
  - [ ] Display scheduled_date below task title (e.g., "Scheduled: Oct 7, 2025")
  - [ ] Show clock icon and scheduled_time if task has both date and time
  - [ ] Use subtle gray background for scheduled tasks to differentiate from unscheduled backlog
  - [ ] Add "Today" / "Tomorrow" / "This Week" quick-filter buttons in Tasks Hub
- [ ] Task 8: Test Daily page and Tasks Hub synchronization (AC: all)
