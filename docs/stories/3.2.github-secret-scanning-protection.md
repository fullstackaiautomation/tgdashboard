...
## Tasks / Subtasks
- [ ] Task 1: Enable GitHub secret scanning (AC: 1)
...
- [ ] Task 2: Set up pre-commit hook with secret detection (AC: 2)
...
- [ ] Task 3: Update .gitignore with comprehensive sensitive patterns (AC: 3)
...
- [ ] Task 4: Audit environment variable usage in codebase (AC: 4)
...
- [ ] Task 5: Create .env.example template file (AC: 5)
...
- [ ] Task 6: Scan git history for accidentally committed secrets (AC: 6)
...
- [ ] Task 7: Clean repository history if secrets found (AC: 7)
...
- [ ] Task 8: Rotate all exposed API keys immediately (AC: 7)
...
- [ ] Task 9: Create local development setup documentation (AC: 8)
...
- [ ] Task 10: Create secret detection test suite (AC: 2, 6)
  ...
  - [ ] Test 1: Create temp file with fake Supabase key → attempt commit → verify blocked
  - [ ] Test 2: Create temp .env file → attempt commit → verify blocked
  - [ ] Test 3: Add hardcoded connection string → attempt commit → verify blocked
  - [ ] Test 4: Add valid code with no secrets → verify commit succeeds
  - [ ] Test 5: Verify pre-commit hook provides helpful error message
  - [ ] Test 6: Test bypass flag: `git commit --no-verify` → verify works (for false positives)
